...
namespace Hostinghelden\HostingheldenTemplate\MetaTag;
...
class MetaTagGenerator
{
    ...
    public function generate(array $params)
    {
        ...
        if (!empty($params['page']['abstract'])) {
            $manager = $metaTagManagerRegistry->getManagerForProperty('abstract');
            $manager->addProperty('abstract', $params['page']['abstract']);
        }

        if (!empty($params['page']['keywords'])) {
            $manager = $metaTagManagerRegistry->getManagerForProperty('keywords');
            $manager->addProperty('keywords', $params['page']['keywords']);
        }

        if (!empty($params['page']['author'])) {
            $manager = $metaTagManagerRegistry->getManagerForProperty('author');
            $manager->addProperty('author', $params['page']['author']);
        }

        if (!empty($params['page']['author_email'])) {
            $manager = $metaTagManagerRegistry->getManagerForProperty('author_email');
            $manager->addProperty('author:email', $params['page']['author_email']);
        }
